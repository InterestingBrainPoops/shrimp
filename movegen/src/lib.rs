...
mod tests {
    ...
    fn perft_numbers() {
        ...
        let seralized: GameRequest = serde_json::from_str(&contents).expect("Invalid json");
        let mut small = seralized.into_small();
        let numbers = perft(&mut small, 5, true, None);
        println!("{}", numbers);
    }

    fn perft(state: &mut SmallRequest, depth: u8, player: bool, you_move: Option<Move>) -> u64 {
        ...
        if player {
            ...
            for x in state.snake_moves(state.you) {
                out += perft(state, depth, !player, Some(x));
            }
            out
        } else {
            let mut out = 1;
            for x in state.all_snake_moves(you_move.unwrap()) {
                ...
                out += perft(state, depth - 1, !player, None);
